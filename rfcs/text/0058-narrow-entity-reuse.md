...
Narrow `entity` reuse so the field set appears only at `entity.*`, `entity.target.*`, `user.entity.*`, `user.target.entity.*`, `host.entity.*`, `host.target.entity.*`, `service.entity.*`, and `service.target.entity.*`.
...
This RFC deletes `cloud` and `orchestrator` from `entity`'s `expected` list and raises `entity` reuse to `order: 3`, then lists `user.target`, `host.target`, and `service.target` explicitly so Graph's `*.target.entity.*` paths are kept. Producers that currently write `cloud.entity.*` or `orchestrator.entity.*` should use root `entity.*` instead. Self-nested copies such as `user.effective.entity.*` and `service.origin.entity.*` should not be populated.
...
- When an event names both an actor and a distinct target, Graph uses `user.target.entity.*`, `host.target.entity.*`, or `service.target.entity.*`.
- When an event names a targeted entity that has no dedicated field set, use `entity.target.*`.
...
`order: 3` runs after the default `order: 2` pass in which `user`, `cloud`, `service`, and `host` copy themselves. Those copies therefore no longer include `entity`. `user.target`, `host.target`, and `service.target` are listed explicitly because they are no longer created automatically.
...
| `host.target.entity.*` | Foreign reuse into `host.target` |
| `service.entity.*` | Foreign reuse into `service` |
| `service.target.entity.*` | Foreign reuse into `service.target` |
...
| `service.origin.entity.*` | Accidental copy; `service.origin` is not in `expected` |
...
**Ingestion:** Beats, Elastic Agent integrations, and ingest pipelines that currently write `cloud.entity.*` or `orchestrator.entity.*` must map those values to root `entity.*`. Pipelines that copied `entity` onto `user.effective`, `user.changes`, `cloud.origin`, `cloud.target`, or `service.origin` should stop doing so. Integrations that already populate `user.entity.*`, `host.entity.*`, `service.entity.*`, `*.target.entity.*`, or root `entity.*` do not need to change.
...
**Why keep `*.target.entity.*` for user, host, and service?** Graph models an actor and a distinct target. `user.target`, `host.target`, and `service.target` already exist as self-nests; listing them in `entity`'s `expected` list is what keeps `user.target.entity.*`, `host.target.entity.*`, and `service.target.entity.*` after `order: 3`. `service.origin.entity.*` is omitted because origin is not part of that Graph pattern.
