...
representing the role of each relevant user. Examples are the `user.target.*` or `user.effective.*` fields. However it's important to note that `user` fields reused within
