...
The default field types in ECS provide the best default experience for most users.
However, a different type from the same type family can replace the default defined in ECS
if required by some use cases. Users should understand any potential performance or storage
differences before changing from a default field type.
