...
For example, both `keyword` and `wildcard` types are members of the `keyword` family, and
`text` and `match_only_text` are members of the `text` family.

The default field types in ECS are selected to provide the best default experience for the
majority of users. However, if your use case requires a the performance or space usage of a
different field type, the default field type may be replaced with a type from the same field
type family.
