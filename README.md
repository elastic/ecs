...
|---|---|---|---|---|---|
| <a name="@timestamp"></a>@timestamp | core | Date/time when the event originated.<br/>For log events this is the date/time when the event was generated, and not when it was read.<br/>Required field for all events. | date |  | `2016-05-23T08:05:34.853Z` |
| <a name="tags"></a>tags | core | List of keywords used to tag each event. | keyword |  | `["production", "env2"]` |
| <a name="labels"></a>labels | core | Key/value pairs.<br/>Can be used to add meta information to events. Should not contain nested objects. All values are stored as keyword.<br/>Example: `docker` and `k8s` labels. | object |  | `{'application': 'foo-bar', 'env': 'production'}` |
| <a name="message"></a>message | core | For log events the message field contains the log message.<br/>In other use cases the message field can be used to concatenate different values which are then freely searchable. If multiple messages exist, they can be combined into one message. | text |  | `Hello World` |
...
| Field  | Level  | Description  | Type  | Multi Field  | Example  |
|---|---|---|---|---|---|
| <a name="agent.version"></a>agent.version | core | Version of the agent. | keyword |  | `6.0.0-rc2` |
| <a name="agent.name"></a>agent.name | core | Name of the agent. | keyword |  | `filebeat` |
| <a name="agent.id"></a>agent.id | core | Unique identifier of this agent (if one exists).<br/>Example: For Beats this would be beat.id. | keyword |  | `8a4f500d` |
| <a name="agent.ephemeral_id"></a>agent.ephemeral_id | extended | Ephemeral identifier of this agent (if one exists).<br/>This id normally changes across restarts, but `agent.id` does not. | keyword |  | `8a4f500f` |
...
| Field  | Level  | Description  | Type  | Multi Field  | Example  |
|---|---|---|---|---|---|
| <a name="cloud.provider"></a>cloud.provider | extended | Name of the cloud provider. Example values are ec2, gce, or digitalocean. | keyword |  | `ec2` |
| <a name="cloud.availability_zone"></a>cloud.availability_zone | extended | Availability zone in which this host is running. | keyword |  | `us-east-1c` |
| <a name="cloud.region"></a>cloud.region | extended | Region in which this host is running. | keyword |  | `us-east-1` |
| <a name="cloud.instance.id"></a>cloud.instance.id | extended | Instance ID of the host machine. | keyword |  | `i-1234567890abcdef0` |
| <a name="cloud.instance.name"></a>cloud.instance.name | extended | Instance name of the host machine. | keyword |  |  |
| <a name="cloud.machine.type"></a>cloud.machine.type | extended | Machine type of the host machine. | keyword |  | `t2.medium` |
| <a name="cloud.account.id"></a>cloud.account.id | extended | The cloud account or organization id used to identify different entities in a multi-tenant environment.<br/>Examples: AWS account id, Google Cloud ORG Id, or other unique identifier. | keyword |  | `666777888999` |
...
| Field  | Level  | Description  | Type  | Multi Field  | Example  |
|---|---|---|---|---|---|
| <a name="container.runtime"></a>container.runtime | extended | Runtime managing this container. | keyword |  | `docker` |
...
| <a name="device.type"></a>device.type | core | The type of the device the data is coming from.<br/>There is no predefined list of device types. Some examples are `endpoint`, `firewall`, `ids`, `ips`, `proxy`. | keyword |  | `firewall` |
...
| <a name="file.extension"></a>file.extension | extended | File extension.<br/>This should allow easy filtering by file extensions. | keyword |  | `png` |
...
| <a name="file.mode"></a>file.mode | extended | Mode of the file in octal representation. | keyword |  | `416` |
...
| <a name="host.os.platform"></a>host.os.platform | extended | Operating system platform (centos, ubuntu, windows, etc.) | keyword |  | `darwin` |
| <a name="host.os.name"></a>host.os.name | extended | Operating system name. | keyword |  | `Mac OS X` |
| <a name="host.os.family"></a>host.os.family | extended | OS family (redhat, debian, freebsd, windows, etc.) | keyword |  | `debian` |
| <a name="host.os.version"></a>host.os.version | extended | Operating system version. | keyword |  | `10.12.6` |
| <a name="host.architecture"></a>host.architecture | core | Operating system architecture. | keyword |  | `x86_64` |
...
| Field  | Level  | Description  | Type  | Multi Field  | Example  |
|---|---|---|---|---|---|
| <a name="http.request.method"></a>http.request.method | extended | Http request method. | keyword |  | `GET, POST, PUT` |
| <a name="http.response.status_code"></a>http.response.status_code | extended | Http response status code. | long |  | `404` |
| <a name="http.response.body"></a>http.response.body | extended | The full http response body. | keyword |  | `Hello world` |
| <a name="http.version"></a>http.version | extended | Http version. | keyword |  | `1.1` |
...
| Field  | Level  | Description  | Type  | Multi Field  | Example  |
|---|---|---|---|---|---|
| <a name="log.level"></a>log.level | core | Log level of the log event.<br/>Some examples are `WARN`, `ERR`, `INFO`. | keyword |  | `ERR` |
...
| Field  | Level  | Description  | Type  | Multi Field  | Example  |
|---|---|---|---|---|---|
| <a name="network.name"></a>network.name | extended | Name given by operators to sections of their network. | keyword |  | `Guest Wifi` |
| <a name="network.protocol"></a>network.protocol | core | Network protocol name. | keyword |  | `http` |
| <a name="network.direction"></a>network.direction | core | Direction of the network traffic.<br/>Recommended values are:<br/>  * inbound<br/>  * outbound<br/>  * unknown | keyword |  | `inbound` |
| <a name="network.forwarded_ip"></a>network.forwarded_ip | core | Host IP address when the source IP address is the proxy. | ip |  | `192.1.1.2` |
| <a name="network.inbound.bytes"></a>network.inbound.bytes | core | Network inbound bytes. | long |  | `184` |
| <a name="network.inbound.packets"></a>network.inbound.packets | core | Network inbound packets. | long |  | `12` |
| <a name="network.outbound.bytes"></a>network.outbound.bytes | core | Network outbound bytes. | long |  | `184` |
| <a name="network.outbound.packets"></a>network.outbound.packets | core | Network outbound packets. | long |  | `12` |
| <a name="network.total.bytes"></a>network.total.bytes | core | Network total bytes. The sum of inbound.bytes + outbound.bytes. | long |  | `368` |
| <a name="network.total.packets"></a>network.total.packets | core | Network outbound packets. The sum of inbound.packets + outbound.packets | long |  | `24` |
...
| Field  | Level  | Description  | Type  | Multi Field  | Example  |
|---|---|---|---|---|---|
| <a name="os.platform"></a>os.platform | extended | Operating system platform (such centos, ubuntu, windows). | keyword |  | `darwin` |
| <a name="os.name"></a>os.name | extended | Operating system name. | keyword |  | `Mac OS X` |
| <a name="os.family"></a>os.family | extended | OS family (such as redhat, debian, freebsd, windows). | keyword |  | `debian` |
| <a name="os.version"></a>os.version | extended | Operating system version as a raw string. | keyword |  | `10.12.6-rc2` |
| <a name="os.kernel"></a>os.kernel | extended | Operating system kernel version as a raw string. | keyword |  | `4.4.0-112-generic` |
...
| Field  | Level  | Description  | Type  | Multi Field  | Example  |
|---|---|---|---|---|---|
| <a name="process.args"></a>process.args | extended | Process arguments.<br/>May be filtered to protect sensitive information. | keyword |  | `['-l', 'user', '10.0.0.16']` |
| <a name="process.name"></a>process.name | extended | Process name.<br/>Sometimes called program name or similar. | keyword |  | `ssh` |
...
| <a name="service.name"></a>service.name | core | Name of the service data is collected from.<br/>The name can be used to group and correlate logs and metrics from one service.<br/>Example: If logs or metrics are collected from Redis, `service.name` would be `redis`. | keyword |  | `elasticsearch` |
...
| <a name="service.version"></a>service.version | core | Version of the service the data was collected from.<br/>This allows to look at a data set only for a specific version of a service. | keyword |  | `3.2.4` |
| <a name="service.ephemeral_id"></a>service.ephemeral_id | extended | Ephemeral identifier of this service (if one exists).<br/>This id normally changes across restarts, but `service.id` does not. | keyword |  | `8a4f500f` |
...
| Field  | Level  | Description  | Type  | Multi Field  | Example  |
|---|---|---|---|---|---|
| <a name="url.href"></a>url.href | extended | Full url. The field is stored as keyword. | keyword |  | `https://elastic.co:443/search?q=elasticsearch#top` |
| <a name="url.scheme"></a>url.scheme | extended | Scheme of the request, such as "https".<br/>Note: The `:` is not part of the scheme. | keyword |  | `https` |
| <a name="url.hostname"></a>url.hostname | extended | Hostname of the request, such as "elastic.co".<br/>In some cases a URL may refer to an IP and/or port directly, without a domain name. In this case, the IP address would go to the `hostname` field. | keyword |  | `elastic.co` |
| <a name="url.port"></a>url.port | extended | Port of the request, such as 443. | integer |  | `443` |
...
| <a name="user_agent.name"></a>user_agent.name | extended | Name of the user agent. | keyword |  | `Chrome` |
