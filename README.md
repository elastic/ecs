...
| <a name="service.name"></a>service.name | Name of the service data is collected from.<br/>The name can be used to group and correlate logs and metrics from one service.<br/>Example: If logs or metrics are collected from Redis, `service.name` would be `redis`. | core | keyword |  | `elasticsearch` |
| <a name="service.type"></a>service.type | Service type. | core | keyword |  |  |
